    ...
    fn parse_subject(&self, table: &mlua::Table) -> Result<Box<dyn Any + Send + Sync>, String> {
        ...
        let storage = match table.get::<mlua::Table>("storage") {
            Ok(t) if t.get::<bool>("flaky").unwrap_or(false) => {
                ...
                crate::ports::components::StorageOpts::from(spec.clone()).validate()?;
                Some(spec)
            }
            _ => None,
        };
...
impl Docker {
    ...
    async fn deprive_resource(
        &self,
        subject: &Subject,
        tier: &crate::domain::fault::Tier,
    ) -> Result<(), String> {
        let id = Self::container_id(subject).to_string();

        match tier {
            crate::domain::fault::Tier::Disk => {
            ...
            crate::domain::fault::Tier::Network => {
